...
class User {
  ...
  static getByName(username, fn) {
    UserModel.findOne({username}).exec((err, user: any) => {
      if (err || !user) return fn(`username: ${username} does't  exits`)
  ...
  static getById(id, fn) {
    User.getUser(id, (err, user) => {
      if (err || !user) return fn(`id: ${name} does't  exits`)
        // https://stackoverflow.com/questions/34775687/express-jwt-setting-user-object-to-req-user-doc-instead-of-just-req-user
        fn(null, new User(user.toObject()))
  ...
  static authenticate(name, psw, fn){User.getByName(
      name,
      function(err, user) {
        if (err) return fn(err)
          bcrypt.genSalt(12)
              .then(salt => {return bcrypt.hash(psw, user.salt)})
              .then(slatPsw => {
  ...
  save(fn: ErrFn) {
    ...
    if (user._id) {
      ...
      user._hashPassword()
          .then((hashd: string) => {
            user.password = hashd
            user.model = new UserModel(
                {username: user.username, password: hashd, salt: user.salt})
